...
public class ContentAudutingTest extends AbstractCOSClientCITest {
    ...
    public void testCreateVideoAuditingJob() {
        try { // 1.创建任务请求对象
            ...
            videoId = response.getJobsDetail()
                    .getJobId();
            System.out.println(response);
        } catch (Exception e) {
        }
    ...
    public void testDescribeAuditingJob() throws InterruptedException {
        try {// 1.创建任务请求对象
            ...
            while (true) {
                ...
                String state = response.getJobsDetail()
                        .getState();
                if ("Success".equalsIgnoreCase(state) || "Failed".equalsIgnoreCase(state)) {
                    ...
                    List<AuditingInfo> auditingInfoList = AuditingResultUtil
                            .getAuditingInfoList(response.getJobsDetail());
    ...
    public void testCreateAudioAuditingJobs() {
        try { // 1.创建任务请求对象
            ...
            request.getInput()
                    .setObject("pron.mp3");
            request.getInput()
                    .setDataId("TestDataId");
            request.getConf()
                    .setDetectType("all");
            request.getConf()
                    .setCallback("http://cloud.tencent.com/");
            // 3.调用接口,获取任务响应对象
            AudioAuditingResponse response = cosclient.createAudioAuditingJobs(request);
            audioId = response.getJobsDetail()
                    .getJobId();
        } catch (Exception e) {
        }
    ...
    public void testCreateAuditingTextJobs() {
        try { // 1.创建任务请求对象
            ...
            request.getInput()
                    .setContent("QUs0NyDogonkvr/lmaggIOa4qeWutuWunSAg5Y+q6KaBOTk4");
            // 2.2设置审核类型参数
            request.getConf()
                    .setDetectType("all");
            // 2.3设置审核模板（可选）
            ...
            request.getConf()
                    .setCallbackVersion(CallbackVersion.Simple);
            // 3.调用接口,获取任务响应对象
            TextAuditingResponse response = cosclient.createAuditingTextJobs(request);
            textId = response.getJobsDetail()
                    .getJobId();
        } catch (Exception e) {
        }
    ...
    public void testCreateAuditingDocumentJobs() {
        ...
        // 2.1.2或直接设置请求内容,文本内容的Base64编码
        request.getInput()
                .setUrl("https://" + bucket + ".cos.ap-chongqing.myqcloud.com/test.docx");
        // 2.2设置审核类型参数
        request.getConf()
                .setDetectType("all");
        // 2.3设置审核模板（可选）
