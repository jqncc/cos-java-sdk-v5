...
public enum CodecUtils {
    ...
    /**
     * Transforms the given string into the given destination byte array truncating each character into a byte and
     * skipping carriage returns and line feeds if any.
     * <p>
     * dmurray: "It so happens that we're currently only calling this method with src.length == dest.length, in which
     * case it works, but we could theoretically get away with passing a smaller dest if we knew ahead of time that src
     * contained some number of spaces. In that case it looks like this implementation would truncate the result."
     * <p>
     ...
     * 
     * @throws IllegalArgumentException if the given decoded position is not a possible value produced via the
     *             respective encoding
     */
