...
public class ServiceUtils {
    ...
    /**
     * Same as {@link #downloadObjectToFile(COSObject, File, boolean, boolean)} but has an additional expected file
     * length parameter for integrity checking purposes.
     *
     * @param expectedFileLength applicable only when appendData is true; the expected length of the file to append to.
     */
    public static void downloadToFile(COSObject cosObject, final File dstfile, boolean performIntegrityCheck,
            final boolean appendData, final long expectedFileLength) {
        ...
        try {
            ...
            if (appendData && actualLen != expectedFileLength) {
                ...
                throw new IllegalStateException("Expected file length to append is " + expectedFileLength
                        + " but actual length is " + actualLen + " for file " + dstfile);
            }
            outputStream = new BufferedOutputStream(new FileOutputStream(dstfile, appendData));
            byte[] buffer = new byte[1024 * 10];
            int bytesRead;
            while ((bytesRead = cosObject.getObjectContent()
                    .read(buffer)) > -1) {
    ...
    /**
     * Downloads an COSObject, as returned from {@link COSClient#getObject(com.qcloud.cos.model.GetObjectRequest)}, to
     * the specified file.
     *
     * @param cosObject The COSObject containing a reference to an InputStream containing the object's data.
     ...
     */
    public static void downloadObjectToFile(COSObject cosObject, final File destinationFile,
            boolean performIntegrityCheck, boolean appendData) {
        downloadToFile(cosObject, destinationFile, performIntegrityCheck, appendData, -1);
    }

    /**
     * Interface for the task of downloading object from COS to a specific file, enabling one-time retry mechanism after
     * integrity check failure on the downloaded file.
     */
    public interface RetryableCOSDownloadTask {
         ...
         *
         * @return The COSObject containing a reference to an InputStream containing the object's data.
         */
         ...
         *
         * @return Boolean value indicating whether this task requires integrity check after downloading the COS object
         *         to file.
         */
    ...
    /**
     * Gets an object stored in COS and downloads it into the specified file. This method includes the one-time retry
     * mechanism after integrity check failure on the downloaded file. It will also return immediately after getting
     * null valued COSObject (when getObject request does not meet the specified constraints).
     *
     * @param file The file to store the object's data in.
     * @param retryableCOSDownloadTask The implementation of SafeCOSDownloadTask interface which allows user to get
     *            access to all the visible variables at the calling site of this method.
     */
    public static COSObject retryableDownloadCOSObjectToFile(File file,
        ...
        do {
            ...
            try {
                ServiceUtils.downloadObjectToFile(cosObject, file, retryableCOSDownloadTask.needIntegrityCheck(),
                        appendData);
